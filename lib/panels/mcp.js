(function initIQPanelMcp(global) {
  ...
  function formatMcpConfigEditor() {
    ...
    try {
      ...
      utils.showToast?.("已格式化 JSON");
    } catch (err) {
      utils.showToast?.("JSON 格式錯誤: " + err.message);
    }
  }
  ...
  async function saveMcpConfig() {
    ...
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      utils.showToast?.("設定必須是 JSON 物件");
      return;
    }
    if (!parsed.mcpServers || typeof parsed.mcpServers !== "object" || Array.isArray(parsed.mcpServers)) {
      utils.showToast?.("必須包含 mcpServers 物件");
      return;
    }
    ...
    try {
      ...
      if (res && res.ok) {
        ...
        renderMcpServers();
        utils.showToast?.("MCP 設定已儲存");
      } else {
        utils.showToast?.("儲存失敗: " + (res?.error || "未知錯誤"));
      }
    } catch (err) {
