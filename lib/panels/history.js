    ...
    sessions.forEach((s) => {
      ...
      item.innerHTML = `
        <div class="history-item-row">
          ...
          <button class="icon-btn history-delete-btn" title="刪除" data-sid="${escapeHtml(sid)}">✕</button>
        </div>
      ...
      item.addEventListener("click", async (e) => {
        ...
        try {
          ...
          if (res) {
            ...
            utils.showToast?.("已恢復對話");
            root._switchPanel?.("chat");
          }
        } catch (err) {
          utils.showToast?.("恢復失敗: " + err.message);
        }
      });
    ...
    listEl.querySelectorAll(".history-delete-btn").forEach((btn) => {
      btn.addEventListener("click", async (e) => {
        ...
        try {
          ...
          utils.showToast?.("刪除失敗: " + err.message);
        }
      });
    });
  }
