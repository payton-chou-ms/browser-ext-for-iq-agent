(function initIQPanelContext(global) {
  ...
  function renderCliContext(ctx) {
    ...
    if (el("ctx-models-list")) {
      el("ctx-models-list").innerHTML = models.length === 0
    ...
    if (el("ctx-tools-list")) {
      el("ctx-tools-list").innerHTML = tools.length === 0
    ...
    if (el("ctx-quota")) {
      ...
      el("ctx-quota").innerHTML = keys.length === 0
