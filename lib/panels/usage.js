(function initIQPanelUsage(global) {
  ...
  async function switchModel(modelName) {
    ...
    renderModelsCard();
    utils.showToast?.(`切換模型中: ${modelName}…`);
    ...
    if (currentSessionId && isConnected()) {
      try {
        ...
        if (res && res.ok) {
          utils.showToast?.(`已切換模型: ${modelName}`);
          utils.debugLog?.("CFG", `Model switched successfully: ${res.modelId || modelName}`);
        } else {
          utils.showToast?.(`模型切換失敗: ${res?.error || "unknown"}`, "error");
        ...
        utils.showToast?.(`模型切換失敗: ${err.message}`, "error");
      ...
      utils.showToast?.(`已選定模型: ${modelName}（下次交談使用）`);
    }
  }
  ...
  function renderQuotaCard() {
    ...
    html += '</div>';
    html += '<p class="text-muted quota-source-note">從 Copilot CLI 取得</p>';
    card.innerHTML = html;
  }
  ...
  function populateModelSelect(models) {
    ...
    const chatState = root.chat?.getState?.() || {};
    let currentModel = chatState.currentModel;
