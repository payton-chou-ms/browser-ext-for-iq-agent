(function initIQPanelTasks(global) {
  ...
  function renderTasksList() {
    ...
    if (runningEl) runningEl.textContent = `🔄 ${running} ${t()("tasks.running", "進行中")}`;
    if (doneEl) doneEl.textContent = `✅ ${done} ${t()("tasks.done", "完成")}`;  
        ...
        if (!detail) {
          ...
          detail.innerHTML = `
            <div class="task-detail-section">
              <label>${t()("tasks.args", "參數")}</label>
              <pre>${escapeHtml(argsStr || "—")}</pre>
            </div>
            <div class="task-detail-section">
              <label>${t()("tasks.result", "結果")}</label>
    ...
    for (const agent of subAgents) {
      ...
      agentEl.innerHTML = `
        <div class="parallel-task-header ${agent.status}">
          ...
          <span class="parallel-task-name">${t()("tasks.subAgentPrefix", "子代理：")}${escapeHtml(agent.name || agent.id)}</span>
  ...
  function renderTasksTimeline() {
    ...
    for (const agent of subAgents) {
      ...
      events.push({ offset, elapsed, icon, name: `${t()("tasks.subAgentPrefix", "子代理：")}${agent.name || agent.id}`, status: agent.status });
    }
  ...
  function bindEvents() {
    document.getElementById("btn-clear-tasks")?.addEventListener("click", () => {
      ...
      renderTasksList();
      const timeline = document.getElementById("tasks-timeline");
      if (timeline) timeline.innerHTML = `<div class="empty-state"><span class="empty-icon">📊</span><p>${t()("tasks.noEvents", "尚無事件")}</p></div>`;
    });
  }
