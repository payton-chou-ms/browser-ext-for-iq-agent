    ...
    function handleInsightAction(btn) {
      ...
      switch (action) {
        ...
        case "draft-reply": {
          ...
          root._sendMessage?.();
          // Track ghost reply achievement (when drafting reply for unreplied email)
          if (action === "draft-reply") {
            trackAchievement("ghost_replied", { from, subject });
          }
          break;
        }
        case "reply": {
        ...
        case "meetprep": {
          ...
          root._sendMessage?.();
          // Track meeting prep achievement
          trackAchievement("meeting_prepped", { title });
          break;
        }
        case "open-doc": {
