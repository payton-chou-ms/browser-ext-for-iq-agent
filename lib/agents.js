(function initIQAgents(global) {
  ...
  function renderAgentPanel() {
    ...
    listEl.innerHTML = agents.map((agent) => {
      ...
      const badgeText = connection.isConnected?.() ? (isBuiltInAgent(agent.id) ? "預設" : "自訂") : "local";
      return `
  ...
  async function selectAgent(agentId, options = {}) {
    ...
    if (options.showToast !== false) {
      utils.showToast?.(`已切換至 ${selected.name}`);
    }
  ...
  async function addCustomAgent() {
    ...
    utils.showToast?.(`已新增 Agent: ${newAgent.name}`);
  }
  ...
  async function deleteSelectedAgent() {
    ...
    renderAgentPanel();
    await resetSessionForAgentChange();
    utils.showToast?.(`已刪除 Agent: ${selected.name}`);
  }
