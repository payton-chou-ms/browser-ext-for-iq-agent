(function initIQChat(global) {
  ...
  function getState() {
    return { currentSessionId, currentModel, chatHistory, stats, tokenDetails, currentTitle, currentUrl, currentType, availableModels, toolCalls, sessionData, subAgents };
  }
  ...
  function createSubAgentCard(agent) {
    const template = document.getElementById("subagent-card-template");
    if (!template) return null;
    const card = template.querySelector(".subagent-card").cloneNode(true);
    const nameEl = card.querySelector(".subagent-name");
    const timerEl = card.querySelector(".subagent-timer");
    const statusEl = card.querySelector(".subagent-status");
    if (nameEl) nameEl.textContent = agent.name || agent.id || "sub-agent";
    if (timerEl) timerEl.textContent = "0.0s";
    if (statusEl) {
      statusEl.className = "subagent-status running";
      statusEl.innerHTML = '<span class="task-spinner"></span>';
    }
    card.dataset.agentId = agent.id || "";

    const startedAt = agent.startTime || Date.now();
    const timerId = setInterval(() => {
      if (!timerEl) return;
  ...
  function updateSubAgentCard(card, status) {
    if (!card) return;
    const timerId = card.__timerId;
    if (timerId) {
      clearInterval(timerId);
    ...
    if (status === "success") card.classList.add("done");
  ...
  function createFleetDashboardCard() {
    const card = document.createElement("div");
    card.className = "fleet-dashboard";
    card.innerHTML = `
      <div class="fleet-header">
        <span>🚀 Fleet Tasks</span>
        <span class="fleet-count">0 / 0</span>
      </div>
      <div class="fleet-total-progress"><div class="fleet-total-progress-bar" style="width:0%"></div></div>
      <div class="fleet-agent-list"></div>
      <div class="fleet-eta"></div>
    `;
    return card;
  }

  function renderFleetDashboard(card, fleetTasks) {
    if (!card) return;
    const tasks = Array.from(fleetTasks.values());
    const total = tasks.length;
    const done = tasks.filter((task) => task.status !== "running").length;
    const pct = total > 0 ? Math.round((done / total) * 100) : 0;
    const running = tasks.filter((task) => task.status === "running").length;

    const countEl = card.querySelector(".fleet-count");
    const totalBarEl = card.querySelector(".fleet-total-progress-bar");
    const listEl = card.querySelector(".fleet-agent-list");
    const etaEl = card.querySelector(".fleet-eta");

    if (countEl) countEl.textContent = `${done} / ${total}`;
    if (totalBarEl) totalBarEl.style.width = `${pct}%`;
    if (etaEl) etaEl.textContent = running > 0 ? `running: ${running}` : "all completed";
    if (!listEl) return;

    listEl.innerHTML = tasks
      .map((task) => {
        const statusIcon = task.status === "running" ? "🔄" : task.status === "error" ? "✕" : "✓";
        const fillClass = task.status === "running" ? "running" : task.status === "error" ? "error" : "success";
        const rowPct = task.status === "running" ? "60" : "100";
        return `
          <div class="fleet-agent-row">
            <div class="fleet-agent-info">
              <span class="fleet-agent-status-icon">${statusIcon}</span>
              <span class="fleet-agent-name">${escapeHtml(task.name || task.id || "task")}</span>
              <span class="fleet-agent-pct">${rowPct}%</span>
            </div>
            <div class="fleet-agent-bar"><div class="fleet-agent-bar-fill ${fillClass}" style="width:${rowPct}%"></div></div>
          </div>
        `;
      })
      .join("");
      ...
      port.onMessage.addListener((msg) => {
        if (msg.type === "STREAM_EVENT") {
          ...
          if (evt.type === "subagent.spawn" || evt.type === "subagent.start" || evtData.subagent?.spawn) {
            ...
            root.panels?.tasks?.startTaskTimer?.();
            root.panels?.tasks?.renderTasksList?.();
            const saCard = createSubAgentCard(agent);
            if (saCard) {
              subAgentCards.set(agent.id, saCard);
              parentEl?.appendChild(saCard);
            }
            showIntentBar(`${localizeRuntimeMessage("子代理執行中")} · ${agent.name}`, "🤖");
          }
          if (evt.type === "subagent.complete" || evt.type === "subagent.done" || evtData.subagent?.complete) {
            ...
            if (found) {
              ...
              root.panels?.tasks?.renderTasksList?.();
              const saCard = subAgentCards.get(found.id) || subAgentCards.get(id);
              updateSubAgentCard(saCard, "success");
            }
            showIntentBar(localizeRuntimeMessage("子代理完成"), "✅");
          }
          ...
          if (evt.type === "fleet.task_start" || evtData.fleet?.task_start) {
            ...
            subAgents.push(fleetEntry);
            fleetTasks.set(fleetEntry.id, fleetEntry);
            root.panels?.tasks?.startTaskTimer?.();
            root.panels?.tasks?.renderTasksList?.();
            if (!fleetCard) {
              fleetCard = createFleetDashboardCard();
              parentEl?.appendChild(fleetCard);
            }
            renderFleetDashboard(fleetCard, fleetTasks);
            showIntentBar(localizeRuntimeMessage("Fleet 任務執行中"), "🚀");
          }
          if (evt.type === "fleet.task_complete" || evtData.fleet?.task_complete) {
            ...
            renderFleetDashboard(fleetCard, fleetTasks);
            showIntentBar(localizeRuntimeMessage("Fleet 任務更新"), "🚀");
          }

          if ((evt.type || "").includes("intent") || evtData.intent || evtData.reasoning || evtData.thinking) {
            const intentText = evtData.intent || evtData.text || evtData.message || evtData.reasoning || evtData.thinking;
            if (intentText) showIntentBar(String(intentText), "🧠");
          }
        ...
        if (msg.type === "STREAM_DONE") {
          ...
          subAgentCards.forEach((card) => {
            const statusEl = card.querySelector(".subagent-status");
            if (statusEl?.classList.contains("running")) updateSubAgentCard(card, "success");
          });
          renderFleetDashboard(fleetCard, fleetTasks);
          root.panels?.tasks?.renderTasksList?.();
          if (!bubble) bubble = createStreamingBotMessage();
        ...
        if (msg.type === "STREAM_ERROR") {
          ...
          subAgentCards.forEach((card) => {
            const statusEl = card.querySelector(".subagent-status");
            if (statusEl?.classList.contains("running")) updateSubAgentCard(card, "error");
          });
          fleetTasks.forEach((task, id) => {
            if (task.status === "running") fleetTasks.set(id, { ...task, status: "error", endTime: Date.now() });
          });
          renderFleetDashboard(fleetCard, fleetTasks);
          root.panels?.tasks?.renderTasksList?.();
          if (!bubble) bubble = createStreamingBotMessage();
