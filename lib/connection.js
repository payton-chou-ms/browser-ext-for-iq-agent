(function initIQConnection(global) {
  ...
  const FULL_INIT_SOURCE = "cold-start";

  function normalizeSource(source) {
    return typeof source === "string" && source.trim().length > 0 ? source.trim() : "manual";
  }
  ...
  function updateConnectionUI(state) {
    ...
    if (btnConnect) btnConnect.style.display = state === "connected" ? "none" : "";
    if (btnDisconnect) btnDisconnect.style.display = state === "connected" ? "" : "none";
  }
  ...
  async function checkConnection(source = "manual") {
    const debugLog = utils.debugLog || console.log;
    const sendToBackground = utils.sendToBackground;
    const triggerSource = normalizeSource(source);
    debugLog("CONN", "checkConnection() called");
    try {
      const res = await sendToBackground({ type: "CHECK_CONNECTION", source: triggerSource });
      ...
      if (res) {
        ...
        if (state === "connected") {
          await onConnected({ source: triggerSource });
        }
      } else {
  ...
  function distributeContextSnapshot(ctxRes) {
  ...
  async function runLightweightSync(source) {
    const debugLog = utils.debugLog || console.log;
    const sendToBackground = utils.sendToBackground;

    debugLog("CONN", `onConnected() lightweight sync (source=${source})`);

    try {
      const ctxRes = await sendToBackground({ type: "GET_CONTEXT" });
      if (ctxRes && !ctxRes.error) {
        distributeContextSnapshot(ctxRes);
        return;
      }
      debugLog("ERR", `lightweight GET_CONTEXT returned error (source=${source})`);
    } catch (err) {
      debugLog("ERR", `lightweight GET_CONTEXT failed (source=${source}):`, err.message);
    }
  }
  ...
  async function onConnected(options = {}) {
    ...
    const runFullInit = source === FULL_INIT_SOURCE;

    if (_onConnectedRunning) return;

    if (!runFullInit) {
      await runLightweightSync(source);
    ...
    try {
      ...
      if (ctxRes && !ctxRes.error) {
        const results = await Promise.allSettled([
          (async () => distributeContextSnapshot(ctxRes))(),
        ]);

        results.forEach((r, idx) => {
          if (r.status === "rejected") {
            const labels = ["context-distribution"];
      ...
      routeRuntimeMessage(msg, {
        onConnectionStateChanged: (connectionMsg) => {
          ...
          if (
            ...
            onConnected({ source: connectionMsg.source || "reconnect" });
          }
        },
