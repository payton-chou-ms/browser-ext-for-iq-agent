  ...
  root.panels.proactiveStateFactory = function createProactiveStateFactory() {
    ...
    function _checkProactiveAchievementFlags() {
      ...
      const ghostKeys = ghosts.filter((g) => g.priority === "critical" || g.priority === "high")
        .map((g) => toNotificationKey("ghost", g));
      const readSet = new Set(proactiveState.readKeys || []);
      ...
      const hasBriefing = Boolean(proactiveState.briefing && (
        proactiveState.briefing.emails?.length ||
        proactiveState.briefing.meetings?.length ||
        proactiveState.briefing.tasks?.length ||
        proactiveState.briefing.mentions?.length
      ));
      const hasDeadlines = Boolean(proactiveState.deadlines?.deadlines?.length);
      const hasGhosts = Boolean(proactiveState.ghosts?.ghosts?.length);
