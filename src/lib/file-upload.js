  ...
  async function addFiles(fileList) {
    ...
    for (const file of fileList) {
      if (file.size > MAX_FILE_SIZE) {
        showToast(`${file.name} ${localizeRuntimeMessage("超過 10MB 限制")}`);
        continue;
      }
      ...
      try {
        ...
        showToast(`${localizeRuntimeMessage("讀取 ")}${file.name}${localizeRuntimeMessage(" 失敗: ")}${err.message}`);
      }
    }
    renderFilePreview();
  }
  ...
  function renderFilePreview() {
    ...
    filePreviewBar.innerHTML = pendingFiles.map((f, i) => {
      ...
      return `<div class="file-chip" data-index="${i}">
        ...
        <button class="file-chip-remove" data-index="${i}" title="${localizeRuntimeMessage("移除")}">✕</button>
      </div>`;
    }).join("");
